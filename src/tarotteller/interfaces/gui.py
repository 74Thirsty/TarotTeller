...
def _preview_subsample_factor(inline_size: int, block_size: int) -> int:
    """Return PhotoImage subsample factor needed to fit preview bounds."""

    if inline_size <= 0 or block_size <= 0:
        return 1

    width_factor = math.ceil(inline_size / _PREVIEW_MAX_WIDTH)
    height_factor = math.ceil(block_size / _PREVIEW_MAX_HEIGHT)
